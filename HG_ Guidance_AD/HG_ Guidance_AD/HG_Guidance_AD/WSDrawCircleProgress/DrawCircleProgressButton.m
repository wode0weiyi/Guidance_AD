...
- (void)startAnimationDuration:(CGFloat)duration withBlock:(DrawCircleProgressBlock )block{
    ...
    self.timeNum = duration;
    self.timer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(hg_countDownTime) userInfo:nil repeats:YES];
    [self.timer fire];
    [self.layer addSublayer:self.progressLayer];
}

- (void)hg_countDownTime{
    [self setTitle:[NSString stringWithFormat:@"跳过%ld",self.timeNum] forState:UIControlStateNormal];
    self.timeNum --;
    if (self.timeNum < 0) {
